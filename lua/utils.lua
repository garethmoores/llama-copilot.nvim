...
-- create a floating vertical window with buffer, and without focus.
local function create_vertical_compl_window(buffer)
  local opts = {
    -- relative = "cursor",
    split = "right",
    -- row = 1,
  ...
  log_debug("Creating window opt: " .. vim.inspect(opts))
  local status, err = pcall(vim.api.nvim_set_option_value, "filetype", vim.bo.filetype, { buf = buffer })
  if not status then
    utils.log_debug("Error while creating window " .. vim.inspect(err))
  end

  local window_focus = false
  return vim.api.nvim_open_win(buffer, window_focus, opts)
end
utils.create_vertical_compl_window = create_vertical_compl_window
